...
# Add options
parser.add_argument('tabfile', help="TABULAR file(s) obtained after using the script parseBlastBestHitLoc.pl in a tab file from a BLAST search")
parser.add_argument("--outputname", "-o", help="Output name set by the user")
parser.add_argument("--HSPminlen", "-l", help="Minimum HSP length for a hit to be kept", default="0", dest="hsp", type=int) # nargs='+' All, and at least one, argument

parser.add_argument("--hitfrac", "-f", help="Use a minimum of 'hit fraction': HSPlength/querylength", default="0", type=float) # nargs='+' All, and at least one, argument

parser.add_argument("--evalue", "-e", help="Minimum e-value for keeping a hit", type=float, default=10)
parser.add_argument("--Nhits", "-n", help="Number of hits to be recovered after filtering", type=int)
parser.add_argument("--filterstring", "-y", help="Keep hits with given string (only valid with -s)")
parser.add_argument("--postscript", "-s", help="The input is the file obtained after using the script parseBlastBestHitLoc.pl", default=False, action='store_true')
...
parser.add_argument('--version', "-v", action='version', version='%(prog)s 2.04')
...
if args.postscript and args.headerinput: 
	headercondition = True
# Normal BLAST F T    | 
elif not args.postscript and args.headerinput:
	headercondition = False
# BLAST with header: F F    | -w
elif not args.postscript and not args.headerinput:
	headercondition = True
# Doug without header: T F	| -s -w
elif args.postscript and not args.headerinput:
	headercondition = False
...
if args.Nhits or args.hitfrac or (args.evalue < 10) or args.filterstring:
	...
	if not args.postscript: 		# If BLAST
		eposition = 9
	...
	for key in sorthitskeys: # For each query sequence
		...
		for hit in hits[key][:]: 	# for each hit of every query 
			if args.postscript and hit[2] == "No hit": # If you have No hits (and are working with Doug's script)
				newline = hit   
				...
				if fraction >= args.hitfrac: 					# If the hit fraction is above the threshold
					if args.postscript and args.filterstring: 	# If the we're dealing with Doug's script output and a string to filter was given
						...
						newline = hit   			# If working with normal output or it's Dougs but there was no string to filter with, then take the hit
						newline.insert(0, key) 		# Put the name of the query at the beginning
						filteredtab.append(newline) 	# Add this new line				
						nhits_counter += 1
...
if args.Nhits and args.verbose:
	print "   Max No. of hits kept per query: %d" % args.Nhits

if (args.evalue < 10) and args.verbose:
	print "   Minimum e-value set to %f" % args.evalue

if args.hitfrac and args.verbose:
	print "   Minimum hit fraction set to %f" % args.hitfrac
...
if args.verbose:
	...
	if args.postscript and (args.Nhits or args.hitfrac or (args.evalue < 10)): 
